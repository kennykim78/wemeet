...
3. **소식 묶음 선택(카테고리별 2~3개 소스)**: 아래 신뢰 소스에서 카테고리마다 **주요 소스 1개 + 보조 소스 1~2개**를 고른다(총 design 2~3개, ai 2~3개). 이미 쓴 URL은 제외.
...
4. **큐레이션 작성 (구조화된 심층 큐레이션 + 핵심 짧은 인용 + 교차 참고)**: 두 글 각각에 대해, **원문 전체를 번역·전재하지 말 것.** 본문은 모두 한국어 원작으로 쓰되, 아래 구조를 따라 분석 비중을 두껍게 한다.
   ...
   5. `<h3>교차 참고</h3>` + `<ul><li><a href="...">출처명: 제목</a> — 한 줄 요약</li></ul>` : 보조 소스 1~2개를 반드시 연결.
   6. `<h3>Wemeet의 관점</h3>` + `<p>` 2~3문장 : 차별화된 의견.
