...
window.INSIGHTS = [
  ...
  },
  {
    ...
    "category": "design",
    "date": "2026-09-23",
    "title": "신경다양성 설계는 결국 모두를 위한 UX",
    "rawTitle": "Neurodivergence: What is it and Why Does it Matter",
    "summary": "NN/g가 신경다양성 사용자가 겪는 실행기능·의사소통·감각 장벽과 신경포용 디자인 5원칙을 정리했다. 접근성 개선이 모든 사용자에게 돌아온다는 근거와 함께 짚는다.",
    "bodyHtml": "<p>닐슨 노먼 그룹(NN/g)이 신경다양성(neurodivergence)을 주제로 한 6분짜리 영상을 공개했다. 발표자 테레즈 페센든은 ADHD·자폐·난독증 등으로 정보를 처리하는 방식이 다른 사용자들이 실제 제품에서 부딪히는 벽을 실행기능·의사소통·감각 세 갈래로 정리한다. 핵심은 이것이 소수를 위한 특수 배려가 아니라는 점이다.</p><blockquote>\"신경포용적 디자인은 단순성, 명료성, 통제, 지원, 공동 창작에 집중함으로써 모든 사용자에게 도움이 된다.\"<cite>Nielsen Norman Group</cite></blockquote><h3>왜 중요한가</h3><p>접근성 논의는 오랫동안 시각·청각·운동 장애에 초점이 맞춰져 있었고, 인지적 차이는 \"복잡해서 어렵다\"는 이유로 뒤로 밀렸다. 하지만 실행기능에 부담을 주는 화면(여러 단계를 한 번에 기억해야 하는 결제 플로우, 예고 없이 사라지는 토스트 메시지)은 마감에 쫓기는 일반 사용자도 똑같이 놓친다. 신경다양성 사용자는 우리 UI의 인지 부하를 가장 먼저 드러내 보여주는 리트머스 시험지에 가깝다.</p><h3>실무 적용</h3><p>다섯 가지 원칙을 화면 단위로 번역하면 이렇다. 한 페이지에 결정 하나만 요구하고(단순성), 은유 대신 문자 그대로의 문구를 쓰며(명료성), 자동 재생·자동 전환·시간제한을 끄거나 늦출 수 있게 하고(통제), 진행 상황과 남은 단계를 항상 화면에 남겨 기억에 의존하지 않게 한다(지원). 마지막으로 페르소나를 상상하는 대신 실제 신경다양성 사용자를 테스트와 설계 과정에 부른다(공동 창작).</p><h3>교차 참고</h3><ul><li><a href=\"https://www.nngroup.com/videos/accessible-solutions-benefit-everyone/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Nielsen Norman Group: Accessible Solutions Benefit Everyone</a> — 특정한 필요를 위해 설계한 해법이 결과적으로 모든 사용자의 사용성을 끌어올린다는 점을 정리한 영상이다.</li><li><a href=\"https://www.nngroup.com/articles/writing-for-lower-literacy-users/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Nielsen Norman Group: Lower-Literacy Users</a> — 문장을 단순화한 제약회사 사이트에서 저문해력 사용자의 과제 성공률이 46%에서 82%로 올랐고, 고문해력 사용자 지표도 함께 개선됐다는 조사 결과를 담았다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 신경포용성을 별도 체크리스트가 아니라 정보 구조 설계 단계의 기본값으로 본다. 웹사이트 제작에서 가장 흔한 실수는 \"설명을 더 붙여 친절하게\" 만드는 것인데, 인지 부담을 줄이는 방향은 대개 그 반대다. 한 화면에서 요구하는 결정의 수를 세어보고 그 숫자를 줄이는 것이 카피를 늘리는 것보다 언제나 효과가 크다.</p>",
    "source": "Nielsen Norman Group",
    "sourceUrl": "https://www.nngroup.com/videos/neurodivergence-what-and-why/",
    "tags": [
      "accessibility",
      "neurodiversity",
      "ux"
  ...
  {
    "id": "2026-09-23-relore-repository-memory",
    "category": "ai",
    "date": "2026-09-23",
    "title": "코딩 에이전트에 저장소의 기억을 심다",
    "rawTitle": "relore - repository memory for coding agents",
    "summary": "허깅페이스가 깃허브 논의와 코드를 연결해 에이전트에게 결정의 맥락을 제공하는 오픈소스 도구 relore를 공개했다.",
    "bodyHtml": "<p>허깅페이스가 코딩 에이전트를 위한 저장소 기억 도구 relore를 아파치 2.0 라이선스로 공개했다. 저장소의 커밋 이력과 깃허브 논의를 색인해 현재 코드와 연결하고, 에이전트가 \"이 코드가 왜 이렇게 됐는지\"를 물어볼 수 있게 만드는 도구다. 작성자 타렉 지아데는 문제의 원인을 지식의 부재가 아니라 지식의 고립으로 진단한다.</p><blockquote>\"그 지식은 이미 존재하지만, 수년간의 깃허브 논의 속에 갇혀 있다.\"<cite>Hugging Face</cite></blockquote><h3>왜 중요한가</h3><p>지금 코딩 에이전트의 실패는 대부분 문법이 아니라 맥락에서 나온다. 이미 논의 끝에 기각된 방식을 다시 제안하거나, 다른 기여자가 진행 중인 수정을 중복으로 작업하는 식이다. relore가 단순 검색과 선을 긋고 메인테이너의 결정과 기여자의 주장을 구분해 색인하며 기계 생성 텍스트를 걸러낸다는 점이 중요한 이유다. 맥락의 양이 아니라 맥락의 권위를 다루기 시작했다는 뜻이다.</p><h3>실무 적용</h3><p>토큰 경제성도 눈에 띈다. 파일 전체를 읽히는 대신 정의만 추려주는 defs 명령이 약 5%의 토큰으로 더 나은 파일 개요를 줬다는 현장 테스트 결과가 실려 있다. 실무에서는 에이전트에게 저장소를 통째로 던지는 습관부터 재검토할 만하다. 이력 색인과 작업용 코드 클론을 하나의 HTTP API로 함께 제공하는 구조라 기존 개발 방식을 바꾸지 않고 붙일 수 있다는 점도 도입 장벽을 낮춘다.</p><h3>교차 참고</h3><ul><li><a href=\"https://blog.google/innovation-and-ai/technology/developers-tools/gemini-api-docsmcp-agent-skills/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Google: Improve coding agents’ performance with Gemini API Docs MCP and Agent Skills</a> — 학습 데이터 컷오프 탓에 에이전트가 낡은 API 코드를 생성하는 문제를 문서 MCP와 에이전트 스킬로 보완한 사례다.</li><li><a href=\"https://huggingface.co/blog/FINAL-Bench/ztc\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: Zero-Token Confidence</a> — 모델에게 확신을 물어보는 대신 내부 상태를 읽어 정오를 판별하면 토큰 생성 없이 0.88 정확도를 얻었다는 실험 보고다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 이 흐름을 \"에이전트 성능 경쟁에서 기억 인프라 경쟁으로\"의 이동으로 읽는다. 프로젝트 문서를 따로 쓰는 것보다, 이미 쌓인 이슈·PR 논의를 검색 가능한 형태로 유지하는 쪽이 투자 대비 효과가 크다는 뜻이기도 하다. 결정의 근거를 커밋 메시지와 이슈에 한 줄이라도 남겨두는 팀이 앞으로 에이전트를 더 잘 쓰게 될 것이다.</p>",
    "source": "Hugging Face",
    "sourceUrl": "https://huggingface.co/blog/huggingface/relore-repository-memory",
    "tags": [
      "coding-agents",
      "developer-tools",
      "open-source"
  ...
  },
  {
    ...
    "category": "design",
    "date": "2026-09-21",
    "title": "보드룸에서 살아남는 UX ROI 계산법",
    "rawTitle": "Building A UX ROI Case That Survives The Boardroom",
    "summary": "경영진을 설득하는 건 감각이 아니라 인과관계다. 투자비·귀속률·선행지표까지 갖춘 UX ROI 논증의 뼈대를 정리했다.",
    "bodyHtml": "<p>스매싱 매거진이 UX 성과를 예산 언어로 번역하는 방법을 다섯 단계로 정리했다. 핵심은 '디자인이 좋아졌다'가 아니라 '얼마를 넣어 얼마를 벌었고, 그게 정말 디자인 때문인가'를 증명하는 절차다. 글은 가상의 B2B SaaS 기업 사례로 투자 11만 7천 달러, 귀속 매출 약 70만 6천 달러, 1년차 ROI 약 5:1, 회수 기간 약 2개월이라는 계산 과정을 끝까지 보여준다.</p><blockquote>\"경영진은 UX를 싫어하는 게 아니라 모호함을 싫어한다.\"<cite>Smashing Magazine</cite></blockquote><h3>왜 중요한가</h3><p>디자인 예산은 늘 '있으면 좋은 것' 취급을 받는다. 그 이유는 대부분 성과 측정이 체류시간·만족도 같은 중간 지표에서 멈추기 때문이다. 이 글이 새로운 지점은 비용 쪽을 정직하게 잡는다는 데 있다. 디자이너 인건비만이 아니라 엔지니어링 시간, 이해관계자 회의 시간, 조율 비용까지 투자액에 넣어야 숫자가 반박당하지 않는다.</p><h3>실무 적용</h3><p>세 가지를 그대로 가져다 쓸 만하다. 첫째, KPI는 착수 전에 이해관계자와 함께 정한다(예: 첫 가치 경험까지 14일 → 7일). 둘째, A/B 테스트나 시계열 분석으로 인과를 분리하고, 동시에 돌아간 마케팅 캠페인이 있다면 성과의 일부만 디자인에 귀속시킨다. 셋째, 선행지표(설정 완료율)와 후행지표(전환·매출)를 짝지어 '메커니즘 → 결과'로 이야기한다. 같은 숫자라도 CFO에게는 비용·리스크로, CMO에게는 획득 효율로 틀을 바꿔 말한다.</p><h3>교차 참고</h3><ul><li><a href=\"https://www.nngroup.com/articles/test-earlier-with-ai/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Nielsen Norman Group: Test Complex Interactions Earlier with AI Prototyping</a> — AI 프로토타이핑으로 복잡한 인터랙션을 더 이른 단계에 검증하면 ROI 계산의 '비용' 항목 자체가 줄어든다.</li><li><a href=\"https://www.nngroup.com/videos/democratizing-user-research/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Nielsen Norman Group: Democratizing User Research</a> — 리서치를 조직 전체로 확산시킬 때 품질을 지키는 리서치옵스·템플릿·교육 요건을 짚는다.</li></ul><h3>Wemeet의 관점</h3><p>ROI 문서를 만들 때 가장 자주 무너지는 지점은 계산식이 아니라 귀속률이다. 우리는 웹사이트 리뉴얼 제안에서 '70%만 디자인 몫'처럼 보수적으로 잡는 편을 권한다. 과장된 5:1보다 방어 가능한 3:1이 다음 분기 예산을 지켜준다. 그리고 KPI는 런칭 후가 아니라 계약서 쓸 때 함께 적어두는 것이 좋다. 끝나고 나서 정하는 지표는 언제나 누군가에게 유리하게 해석된다.</p>",
    "source": "Smashing Magazine",
    "sourceUrl": "https://www.smashingmagazine.com/2026/09/building-ux-roi-case-survives-boardroom/",
    "tags": [
      "ux-roi",
      "design-metrics",
      "stakeholders"
  ...
  {
    "id": "2026-09-21-google-ai-economy-atlas",
    "category": "ai",
    "date": "2026-09-21",
    "title": "구글 AI 경제 지도가 드러낸 나라별 격차",
    "rawTitle": "New insights from Google's AI & Economy ATLAS",
    "summary": "인도는 디자인·미디어, 미국은 개발·수학. 구글 ATLAS 업데이트는 AI 사용이 나라마다 전혀 다른 직군으로 쏠린다는 걸 보여준다.",
    "bodyHtml": "<p>구글이 AI &amp; Economy ATLAS에 새 데이터 시각화와 연구를 더했다. 눈에 띄는 건 총량이 아니라 분포다. 인도에서는 예술·디자인·미디어 직군이 업무 관련 AI 사용의 19%를 차지해 세계 평균의 1.6배였고, 미국은 컴퓨터·수학 직군이 30%로 세계 평균의 두 배였다. 브라질과 독일은 수작업 관련 활용이 7%로 일본(4%)보다 높았다. 구글·구글 딥마인드·MIT FutureTech가 함께 수행한 조사에서는 과학자의 절반 가까이가 매일 AI를 쓴다고 답했다.</p><blockquote>\"과학자들은 AI 덕분에 주당 7시간에 조금 못 미치는 시간을 아끼고 있다고 보고한다.\"<cite>Google Blog</cite></blockquote><h3>왜 중요한가</h3><p>'AI 도입률 몇 퍼센트'라는 한 줄 숫자는 실제로는 거의 아무것도 설명하지 못한다. 같은 도입률이라도 그 나라가 AI를 코드 작성에 쓰는지, 비주얼 제작에 쓰는지에 따라 산업 구조에 남는 자국이 완전히 다르다. ATLAS의 직군별 분해는 그 차이를 처음으로 비교 가능한 형태로 만들어준다.</p><h3>실무 적용</h3><p>해외 클라이언트를 상대하는 팀이라면 이 분포를 제안 전략에 반영할 수 있다. 크리에이티브 직군의 AI 활용이 높은 시장에서는 'AI로 제작 속도를 올린다'는 메시지가 이미 상식이라 차별점이 되지 않고, 오히려 품질 검수와 브랜드 일관성이 설득 포인트가 된다. 반대로 기술 직군 중심 시장에서는 파이프라인 통합과 자동화 범위를 보여주는 쪽이 통한다. 사내 도입을 설계할 때도 '전사 교육'보다 실제 사용이 몰리는 직군부터 파고드는 편이 투자 대비 효율이 높다.</p><h3>교차 참고</h3><ul><li><a href=\"https://blog.google/innovation-and-ai/technology/research/understanding-the-ai-economy/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Google Blog: Understanding the AI economy</a> — 첫 ATLAS 보고서. 전형적인 직무에서 AI가 쓰이는 업무는 약 21%에 그치고, 그중 완전 자동화는 10% 미만이라고 짚었다.</li><li><a href=\"https://huggingface.co/blog/state-of-open-models-summer-2026\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: State of Open Models: Summer 2026 Observations</a> — 화제는 프런티어 모델이 가져가지만 2026년 다운로드의 97%는 1B 미만 소형 모델이 차지했다.</li></ul><h3>Wemeet의 관점</h3><p>두 보고서를 겹쳐 보면 현장의 AI는 '대체'가 아니라 '부분 가속'에 가깝다. 업무의 5분의 1에서만 쓰이고, 그마저도 대부분 사람과 함께 돌아간다. 그래서 우리는 AI 도입 상담에서 전면 자동화 로드맵을 먼저 그리지 않는다. 주당 몇 시간이 어디서 어떻게 절약되는지 한 직군에서 먼저 증명하고, 그 숫자를 근거로 범위를 넓히는 순서가 훨씬 덜 깨진다.</p>",
    "source": "Google Blog",
  ...
  },
  {
    ...
    "category": "design",
    "date": "2026-09-18",
    "title": "버튼의 종말: 인터페이스 없는 인터페이스",
    "rawTitle": "The Death Of The Button: Why The Best Interface Is No Interface",
    "summary": "클릭과 메뉴 대신 사용자의 의도를 받아 AI가 실행하는 방향으로 웹 인터페이스가 이동하고 있다.",
    "bodyHtml": "<p>Smashing Magazine에 Carrie Webster가 기고한 글은 웹 디자인의 무게중심이 '보이는 화면'에서 '의도를 처리하는 시스템'으로 옮겨가고 있다고 주장합니다. 사용자가 여러 단계의 폼과 메뉴를 거치는 대신 목표를 한 번 말하면, 시스템이 뒤에서 작업을 수행하는 구조가 다음 표준이 된다는 것입니다. 글은 Perplexity, Vercel v0, Goblin.tools를 예로 들며 이 변화를 설명합니다.</p><blockquote>\"현대 웹 디자인의 궁극적 목표는 더 예쁜 버튼이 아니라 인터페이스 자체를 없애는 것이다.\"<cite>Smashing Magazine</cite></blockquote><h3>왜 중요한가</h3><p>글이 제시하는 세 축은 상위 수준의 목표 입력, 상황에 맞게 바뀌는 생성형 UI, 그리고 쉽게 되돌릴 수 있는 자율 실행입니다. 이는 화면 설계 중심이던 UX의 산출물이 '흐름과 권한, 복구 경로' 설계로 바뀐다는 뜻입니다. 버튼이 사라져도 결정은 사라지지 않고, 사용자가 보지 못하는 곳으로 이동할 뿐입니다.</p><h3>실무 적용</h3><p>의도 기반 기능을 넣을 때는 입력창 하나보다 '실행 전 확인, 실행 후 되돌리기, 진행 상황 노출' 세 가지 장치를 먼저 설계해야 합니다. 특히 결제·예약·데이터 수정처럼 되돌리기 어려운 작업은 여전히 명시적인 확인 버튼이 필요합니다. 모든 흐름을 대화형으로 바꾸기보다, 반복적이고 단계가 많은 작업부터 의도 입력으로 압축하는 편이 현실적입니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://www.smashingmagazine.com/2026/08/why-website-should-never-stop-changing/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Smashing Magazine: Why Your Website Should Never Stop Changing</a> — 출시 후에도 에이전트가 지속적으로 사이트를 최적화하는 '자율형 웹사이트' 개념을 다룹니다.</li><li><a href=\"https://www.nngroup.com/articles/ai-ux-debt/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">NN/g: The Custodial Era of UX: Cleaning Up After AI</a> — AI가 빠르게 만든 경험이 남기는 UX 부채를 디자이너가 수습하게 되는 현실을 짚습니다.</li></ul><h3>Wemeet의 관점</h3><p>'인터페이스 없는 인터페이스'는 디자인이 줄어드는 게 아니라 보이지 않는 부분의 설계 책임이 커진다는 의미로 읽어야 합니다. Wemeet은 의도 기반 기능을 제안할 때 화면 시안과 함께 실패·취소·되돌리기 시나리오를 같은 비중으로 설계합니다. 사용자가 통제권을 잃지 않는다는 확신이 있어야 버튼을 줄일 수 있습니다.</p>",
    "source": "Smashing Magazine",
    "sourceUrl": "https://www.smashingmagazine.com/2026/09/death-button-why-best-interface-is-no-interface/",
    "tags": [
      "Intent-Driven UI",
      "Generative UI",
      "UX Strategy"
  ...
  {
    "id": "2026-09-18-openai-agents-api-sandboxes",
    "category": "ai",
    "date": "2026-09-18",
    "title": "OpenAI Agents API 공개, 에이전트 인프라의 표준화",
    "rawTitle": "Introducing the Agents API",
    "summary": "Codex를 움직이는 하네스와 호스팅 샌드박스를 API로 공개해, 장기 실행 에이전트 구축 부담을 크게 낮췄다.",
    "bodyHtml": "<p>OpenAI가 Agents API를 퍼블릭 베타로 공개했습니다. Codex를 구동하는 에이전트 하네스와 인프라를 개발자에게 그대로 열어준 것으로, 별도 이용료 없이 사용한 토큰과 도구 비용만 내면 됩니다. 함께 발표된 OpenAI 호스팅 샌드박스에서는 에이전트가 코드를 실행하고 파일을 다루며 결과물을 만들어낼 수 있습니다.</p><blockquote>\"Agents API는 프로덕션에서 지속적으로 동작하는 에이전트를 위한 내구성 있는 세션·오케스트레이션 계층을 제공한다.\"<cite>OpenAI</cite></blockquote><h3>왜 중요한가</h3><p>API는 에이전트(모델·지시·도구·MCP 서버), 환경(샌드박스), 세션(작업 단위의 지속 인스턴스), 이벤트라는 네 개념으로 구성됩니다. 컨텍스트 자동 압축, 멀티 에이전트 오케스트레이션, 프로그래밍 방식의 도구 호출까지 기본 제공되어, 그동안 팀마다 직접 만들던 '에이전트 런타임'이 플랫폼 기능으로 흡수되고 있습니다.</p><h3>실무 적용</h3><p>사내 자동화나 고객용 AI 기능을 만들 때 이제 관건은 인프라가 아니라 어떤 작업을 어떤 권한으로 맡길지 정의하는 일입니다. 세션이 오래 유지되는 만큼 로그·비용·권한 범위를 처음부터 설계해야 하며, 기존 MCP 서버를 그대로 연결할 수 있어 도구 자산의 재사용성이 높아집니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://huggingface.co/blog/sergiopaniego/rl-environments-2026\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: One sandbox per rollout, or how labs run RL for agents in 2026</a> — 주요 연구소들이 에이전트 학습 시도마다 격리된 샌드박스를 띄우는 방식으로 RL 인프라를 재편한 흐름을 정리합니다.</li><li><a href=\"https://developers.openai.com/api/docs/guides/agents-api/overview\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">OpenAI API Docs: Agents API overview</a> — 에이전트·환경·세션·이벤트의 구조와 사용법을 설명하는 공식 가이드입니다.</li></ul><h3>Wemeet의 관점</h3><p>학습 단계에서도, 서비스 단계에서도 '샌드박스 안의 에이전트'가 공통 구조로 자리 잡고 있습니다. 모델 성능 경쟁만큼 환경 설계가 품질을 좌우하는 시대이므로, Wemeet은 AI 기능 기획 시 에이전트가 일할 환경과 검증 기준부터 정의하는 방식을 권합니다.</p>",
    "source": "OpenAI",
  ...
  },
  {
    ...
    "category": "design",
    "date": "2026-09-05",
    "title": "AI 시대의 UX, 무엇을 다시 배울 것인가",
    "rawTitle": "Using AI for UX Work: Study Guide",
    "summary": "NN/g가 UX 실무에 AI를 녹여내는 학습 가이드를 냈다. AI는 대체가 아니라 조력자이며, 통합·경력·리서치 등 7개 영역으로 길을 정리했다.",
    "bodyHtml": "<p>Nielsen Norman Group의 Tanner Kohler가 UX 실무자를 위한 &lsquo;AI 활용 학습 가이드&rsquo;를 공개했습니다. 요지는 분명합니다 &mdash; AI는 디자이너를 대체하는 도구가 아니라, 기존 역량을 넓혀 주는 조력자로 다뤄야 한다는 것입니다. 가이드는 흩어진 자료를 한데 모아, 어디서부터 무엇을 익힐지 지도를 그려 줍니다.</p><blockquote>\"예전이 어땠는지 한탄하기보다, AI 시대에도 가치를 유지하도록 당신의 기술과 사고방식을 바꿔라.\"<cite>Tanner Kohler, Nielsen Norman Group</cite></blockquote><h3>왜 중요한가</h3><p>가이드는 UX 업무를 일곱 갈래로 나눕니다 &mdash; 워크플로에 AI 통합하기, 경력 전략과 역할 진화, 디자인, 리서치, 라이팅, 서비스 디자인, 그리고 아이디에이션·워크숍입니다. 마지막 절은 &lsquo;지금 이 시점&rsquo;의 AI 실무 현황을 시점을 밝혀 정리해, 빠르게 바뀌는 지식의 유통기한을 드러냅니다. 핵심 메시지는 &lsquo;AI가 일자리를 없앤다&rsquo;가 아니라, 표면 작업이 상품화될수록 전략·판단 같은 깊은 역량이 차별화가 된다는 점입니다.</p><h3>실무 적용</h3><p>실무자는 이 지도를 &lsquo;체크리스트&rsquo;가 아니라 &lsquo;학습 순서&rsquo;로 써야 합니다 &mdash; 프롬프트 구조(예: CARE 프레임)로 결과의 편차를 줄이고, 리서치는 AI로 &lsquo;가속&rsquo;하되 실제 사용자 증거를 대체하지는 않도록 경계를 긋는 식입니다. 팀 차원에서는 각 구성원이 어느 갈래에서 약한지 진단해, 세대별 자료·팟캐스트·코스로 개인화된 학습 경로를 짜는 데 이 프레임이 유용합니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://www.nngroup.com/articles/state-of-ux-2026/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">NN/G: State of UX 2026 &mdash; Design Deeper to Differentiate</a> &mdash; UI가 더는 차별화가 아니며 전략·리서치·리더십을 결합한 &lsquo;깊은 사고&rsquo;가 경쟁력이라는 큰 그림을 제시합니다.</li><li><a href=\"https://www.nngroup.com/articles/ai-ux-debt/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">NN/G: The Custodial Era of UX &mdash; Cleaning Up After AI</a> &mdash; AI가 빠르게 만들어 낸 결과를 UX가 뒤에서 정리·검수해야 하는 새 국면을 짚어, 무엇을 익혀 둬야 하는지 방향을 보탭니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 AI 도구 목록을 외우는 것과, 그 도구를 언제·왜 쓸지 판단하는 것은 전혀 다른 역량이라고 봅니다 &mdash; 전자는 반년이면 낡지만 후자는 쌓일수록 값이 오릅니다. AI를 조력자로 세우고 판단의 근육을 키우는 팀이, 표면 작업이 흔해진 시대에도 대체되지 않습니다.</p>",
    "source": "Nielsen Norman Group",
    "sourceUrl": "https://www.nngroup.com/articles/ai-work-study-guide/",
    "tags": [
      "UX",
      "AI Workflow",
      "Career"
  ...
  {
    "id": "2026-09-05-benchmirt-llm-benchmarks",
    "category": "ai",
    "date": "2026-09-05",
    "title": "LLM 벤치마크는 정말 무엇을 재고 있나",
    "rawTitle": "BenchMIRT: What are LLM benchmarks actually measuring?",
    "summary": "Allen AI가 100개 모델·16개 벤치마크·3.4만 문항을 분석하니, 점수 뒤엔 '안전'과 '일반 추론' 두 축이 숨어 있었다. 문항의 10%만 남겨도 순위가 유지됐다.",
    "bodyHtml": "<p>Allen AI(Ai2)가 &lsquo;벤치마크 점수는 대체 무슨 능력을 재는가&rsquo;를 정면으로 파고든 도구 BenchMIRT를 공개했습니다. 100개 언어 모델을 16개 벤치마크의 3만 4천여 문항에 걸쳐 다차원 분석한 결과, 개별 벤치마크가 겉으로 내건 목표와 실제로 재는 능력이 자주 어긋난다는 사실이 드러났습니다.</p><blockquote>\"벤치마크는 보통 안전, 일반 추론, 지시 따르기 같은 특정 능력 하나를 재도록 설계된다.\"<cite>Allen AI (Ai2), Hugging Face</cite></blockquote><h3>왜 중요한가</h3><p>흥미로운 대목은 BenchMIRT가 &lsquo;어느 벤치마크가 무엇을 잰다&rsquo;는 사전 지시 없이도 두 개의 지배적 축 &mdash; 안전과 일반 추론 &mdash; 을 스스로 찾아냈다는 점입니다. 그리고 그 축으로 보면 라벨이 배신합니다 &mdash; 사회적 편향을 잰다는 BBQ가 오히려 일반 추론에 가깝게 정렬됐고, 위험 지식을 잰다는 WMDP도 안전보다 추론 능력과 더 상관했습니다. &lsquo;안전 점수가 높다&rsquo;는 말이 실은 &lsquo;추론을 잘한다&rsquo;였을 수 있다는 뜻입니다.</p><h3>실무 적용</h3><p>가장 실용적인 발견은 &lsquo;중복&rsquo;입니다 &mdash; 문항의 10%만 남겨도 모델 순위가 대체로 그대로 유지됐습니다. 평가 비용과 시간이 곧 돈인 팀에게 이는 곧, 무작정 큰 벤치마크를 돌리는 대신 신호가 겹치지 않는 핵심 문항만 골라 &lsquo;압축 평가셋&rsquo;을 꾸릴 수 있다는 실전 지침이 됩니다. 나아가 하나의 점수를 맹신하지 말고, 그 점수가 안전인지 추론인지 축을 분해해 읽는 습관이 필요합니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://www.nngroup.com/articles/eval-ai-output/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">NN/G: One AI Output Is an Example, Not an Evaluation</a> &mdash; 결과 하나를 성능의 증거로 착각하지 말라는 경고로, &lsquo;무엇을·어떻게 평가할 것인가&rsquo;라는 이번 연구의 문제의식과 맞닿습니다.</li><li><a href=\"https://huggingface.co/blog/state-of-open-models-summer-2026\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: State of Open Models &mdash; Summer 2026</a> &mdash; 오픈 모델이 쏟아지는 흐름 속에서 &lsquo;믿을 만한 평가&rsquo;가 왜 병목이 되는지 큰 맥락을 제공합니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 AI 도입의 진짜 리스크가 &lsquo;낮은 점수&rsquo;가 아니라 &lsquo;잘못 읽은 점수&rsquo;라고 봅니다 &mdash; 벤치마크가 무엇을 재는지 모른 채 순위만 좇으면, 안전을 샀다고 믿으며 추론력을 산 셈이 됩니다. 점수의 축을 분해해 읽고, 우리 과업에 맞는 압축 평가셋을 직접 설계하는 팀이 AI를 안전하게 굴립니다.</p>",
    "source": "Hugging Face",
  ...
  {
    "id": "2026-09-04-eu-ai-labelling-guidelines",
    "category": "design",
    "date": "2026-09-04",
    "title": "EU의 AI 표시 의무, 디자이너가 할 일",
    "rawTitle": "New EU Guidelines For AI Labelling",
    "summary": "EU의 AI 투명성 규정이 8월 2일 발효됐다. 딥페이크·챗봇·순수 AI 텍스트는 표시 대상이며, 반짝이 아이콘 하나로는 부족하다.",
    "bodyHtml": "<p>유럽연합의 AI 투명성 규정이 2026년 8월 2일부터 발효됐습니다. 전면 금지가 아니라, AI가 만든 콘텐츠가 사람의 창작물로 오인될 수 있을 때 &lsquo;분명히 밝히라&rsquo;는 공개(disclosure) 의무가 골자입니다. Vitaly Friedman은 무엇을 표시해야 하고 무엇은 면제되는지, 그리고 그 표시를 &lsquo;어떻게&rsquo; 해야 하는지를 정리했습니다.</p><blockquote>\"AI 생성물이 사람의 창작물로 오인될 수 있다면, 그 표시는 명확하고 구별 가능(clear and distinguishable)해야 한다.\"<cite>Vitaly Friedman, Smashing Magazine</cite></blockquote><h3>왜 중요한가</h3><p>표시 대상은 넓습니다 &mdash; 실존 인물·장소처럼 보이는 딥페이크, 챗봇·AI 에이전트, 사람의 편집 검토 없이 공적 사안을 다룬 순수 AI 작성 텍스트, 감정 인식·생체 분류 도구가 모두 포함됩니다. 이 흐름은 EU만의 것이 아닙니다 &mdash; 중국(2025년 9월 1일), 캘리포니아(2026년 8월 2일), 한국(2026년 1월 22일), 인도(2026년 2월 20일)가 비슷한 규정을 이미 시행 중입니다. 표시는 이제 &lsquo;해도 되는 배려&rsquo;가 아니라 &lsquo;안 하면 위반&rsquo;인 설계 요건이 됐습니다.</p><h3>실무 적용</h3><p>핵심은 &lsquo;반짝이 아이콘(sparkle icon) 하나로는 부족하다&rsquo;는 점입니다 &mdash; EU 집행위는 공식 AI 아이콘과 함께 &lsquo;AI 생성(AI-generated)&rsquo; 같은 평문 라벨을 병기하고, 콘텐츠가 재공유·다운로드돼도 그 표시가 사라지지 않게 하라고 권고합니다. 반대로 실명이 드러난 담당자가 실질적으로 편집·검수한 콘텐츠, 맞춤법·문법·서식 같은 사소한 수정, AI 번역은 표시 의무에서 면제됩니다. 디자이너는 라벨을 나중에 덧붙이는 장식이 아니라, 컴포넌트와 메타데이터 수준에서 지속되는 요소로 설계해야 합니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://www.nngroup.com/articles/artificial-intelligence-glossary/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">NN/G: Artificial Intelligence &mdash; Glossary</a> &mdash; 에이전트·토큰·프롬프트 인젝션 등 규정과 실무를 잇는 AI 용어를 평문으로 정의해, 무엇을 &lsquo;AI 생성&rsquo;으로 볼지 공통 언어를 제공합니다.</li><li><a href=\"https://www.nngroup.com/articles/ai-generated-images/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">NN/G: AI-Generated Images Can Perform as Well as Stock Photography</a> &mdash; 사용자가 AI 생성 여부를 모를 때 인식상 불이익이 없었다는 연구로, &lsquo;그렇다면 무엇을·어떻게 밝힐 것인가&rsquo;라는 이번 규정의 질문과 맞닿습니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 AI 표시를 규제 대응이 아니라 신뢰 설계로 봅니다 &mdash; 잘 만든 공개는 콘텐츠의 값을 떨어뜨리지 않고 오히려 출처의 투명성으로 신뢰를 쌓습니다. 재공유돼도 살아남는 라벨을 처음부터 컴포넌트에 심는 팀이, 규제와 사용자 신뢰를 동시에 얻습니다.</p>",
    "source": "Smashing Magazine",
  ...
  {
    "id": "2026-09-04-own-your-agent-memory",
    "category": "ai",
    "date": "2026-09-04",
    "title": "코딩 에이전트에 '내 것'인 기억을 심다",
    "rawTitle": "Give Your Coding Agents a Memory You Own",
    "summary": "Hugging Face에 공개된 funes가 에이전트 세션을 로컬에 색인해 소유 가능한 기억으로 만든다. 리콜이 직접 인수인계보다 4~8배 저렴했다.",
    "bodyHtml": "<p>코딩 에이전트를 바꿀 때마다, 혹은 다른 컴퓨터로 옮길 때마다 지난 세션의 판단과 맥락이 통째로 사라집니다. Hugging Face에 공개된 David Corvoysier의 &lsquo;funes&rsquo;는 이 문제를 &lsquo;내가 소유하는 기억 계층&rsquo;으로 풉니다 &mdash; 에이전트의 세션 기록을 로컬에 색인해, 여러 에이전트와 기기를 넘나드는 검색 가능한 기억으로 만듭니다.</p><blockquote>\"리콜(recall) 기반 기억은 한 작업에서 직접 작성한 인수인계보다 8배, 다른 작업에서는 4배 저렴했다.\"<cite>David Corvoysier, Hugging Face</cite></blockquote><h3>왜 중요한가</h3><p>설계의 요점은 &lsquo;로컬 우선(local-first)&rsquo;과 &lsquo;소유&rsquo;입니다 &mdash; 임베딩과 리랭킹이 클라우드가 아니라 사용자 기기에서 돌고, 기억은 기본값이 비공개인 Hugging Face 데이터셋으로 사용자가 완전히 소유합니다. Claude Code, Codex, pi, Hermes 등 여러 에이전트가 같은 기억을 공유하므로, 도구를 갈아타도 그동안 쌓인 판단이 따라옵니다. 8배·4배라는 비용 절감은 부수 효과가 아니라, 인수인계 문서를 매번 다시 쓰던 낭비를 없앤 결과입니다.</p><h3>실무 적용</h3><p>구조는 실용적입니다 &mdash; ML 런타임 의존성 없이 <code>curl</code>로 단일 바이너리만 설치하고, 추가·불변(append-only)인 Lance 데이터셋에 벡터와 BM25를 결합한 하이브리드 검색 위에 크로스 인코더 리랭킹을 얹었습니다. 에이전트는 <code>recall</code> 도구로 관련 세션을 스스로 끌어오고, 사람은 <code>ask</code>로 직접 질의합니다. 색인 전 자격증명을 자동 삭제(redaction)하고 새 세션만 증분 색인하므로, 팀은 민감정보 유출 걱정 없이 &lsquo;기억&rsquo;을 자산으로 쌓을 수 있습니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://huggingface.co/blog/ibm-research/altk-evolve-hmm\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: How Much Memory Does Your Agent Actually Need?</a> &mdash; 에이전트에게 필요한 기억의 &lsquo;양&rsquo;을 따져, 무엇을 남기고 무엇을 버릴지 설계 기준을 제시합니다.</li><li><a href=\"https://huggingface.co/blog/huggingface-hub-v1\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: huggingface_hub v1.0</a> &mdash; 기억을 Hub 데이터셋으로 소유·이식한다는 funes의 전제를 뒷받침하는, 데이터 소유·이동성의 기반을 정리합니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 AI 에이전트의 진짜 경쟁력이 모델 성능보다 &lsquo;축적된 맥락&rsquo;에 있다고 봅니다 &mdash; 세션마다 처음부터 다시 시작하는 팀과, 지난 판단을 소유하고 재사용하는 팀의 격차는 시간이 갈수록 벌어집니다. 벤더에 종속되지 않고 내가 소유하는 기억을 설계하는 것 &mdash; 그것이 에이전트를 도구에서 동료로 바꿉니다.</p>",
    "source": "Hugging Face",
  ...
  },
  {
    ...
    "category": "design",
    "date": "2026-09-02",
    "title": "짧아진 시대에도 롱폼 콘텐츠는 여전히 필요하다",
    "rawTitle": "We Still Need Long-Form Copy",
    "summary": "Nielsen Norman Group이 주의 지속시간이 짧아진 시대에도 롱폼 콘텐츠가 필요한 이유를 짚었다. 복잡한 주제, 신뢰 형성, 그리고 SEO·AEO 성능이 그 근거다.",
    "bodyHtml": "<p>Taylor Dykes가 Nielsen Norman Group에서 &lsquo;주의 지속시간이 짧아졌으니 짧게 써야 한다&rsquo;는 통념에 반론을 폈습니다. 모든 것을 짧게 줄이는 것이 늘 정답은 아니라는 것입니다. 어떤 주제는 짧은 글로는 담기지 않고, 어떤 콘텐츠는 신뢰를 쌓기 위해 충분한 분량을 필요로 하며, 검색과 AI 답변 노출에서도 긴 글이 유리하게 작동합니다.</p><blockquote>\"어떤 것은 복잡하고, 어떤 것은 신뢰가 필요하며, SEO와 AEO의 이점이 있기에 우리는 여전히 롱폼 콘텐츠를 쓴다.\"<cite>Taylor Dykes, Nielsen Norman Group</cite></blockquote><h3>왜 중요한가</h3><p>핵심은 분량이 아니라 &lsquo;깊이가 필요한 순간&rsquo;을 가리는 판단입니다 &mdash; 복잡한 개념은 압축만으로는 오해를 낳고, 사용자가 결정을 내리기 전 확신을 얻으려면 근거와 맥락을 펼쳐 보여야 합니다. 특히 검색엔진 최적화(SEO)에 더해, AI가 답을 길어올리는 원천이 되는 AEO(AI Engine Optimization) 관점에서 충실한 롱폼은 인용될 재료 그 자체가 됩니다. 얕은 콘텐츠는 사람에게도 AI에게도 근거로 선택받지 못합니다.</p><h3>실무 적용</h3><p>롱폼의 적은 &lsquo;길이&rsquo;가 아니라 &lsquo;벽 같은 텍스트&rsquo;입니다 &mdash; 요약 문단, 소제목, 불릿, 시각 자료로 훑어 읽기(scanning)를 설계하면 분량과 가독성을 동시에 잡을 수 있습니다. 도입부에 핵심 요약을 앞세워 급한 독자를 붙잡고, 본문은 구조화된 섹션으로 나눠 사람과 AI 모두가 필요한 조각을 정확히 뽑아가게 만드는 것이 핵심입니다. 분량을 줄이는 대신 스캔 가능성을 높이십시오.</p><h3>교차 참고</h3><ul><li><a href=\"https://www.smashingmagazine.com/2026/08/why-website-should-never-stop-changing/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Smashing Magazine: Why Your Website Should Never Stop Changing</a> &mdash; 콘텐츠를 살아 있게 유지하는 것이 왜 사이트의 생명력인지 짚어, 롱폼의 지속적 갱신 필요성과 맞닿습니다.</li><li><a href=\"https://www.smashingmagazine.com/2026/07/bull-and-bear-case-digital-design-age-ai/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Smashing Magazine: The Bull And Bear Case For Digital Design In The Age Of AI</a> &mdash; AI 시대에 콘텐츠와 디자인의 값이 어디에서 나오는지에 대한 상반된 시나리오를 제시합니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 AI 검색 시대일수록 얕은 요약이 아니라 &lsquo;인용될 만한 깊이&rsquo;가 경쟁력이라고 봅니다. 롱폼은 길게 쓰는 기술이 아니라 깊이를 스캔 가능하게 구조화하는 설계입니다 &mdash; 사람과 AI 모두에게 근거로 선택받는 콘텐츠를 만드는 팀이 검색과 신뢰를 함께 얻습니다.</p>",
    "source": "Nielsen Norman Group",
    "sourceUrl": "https://www.nngroup.com/videos/we-still-need-long-form-copy/",
    "tags": [
      "Content Strategy",
      "SEO",
      "AEO"
  ...
  {
    "id": "2026-09-02-production-parity-replay",
    "category": "ai",
    "date": "2026-09-02",
    "title": "모델 교체, 벤치마크 말고 실제 대화로 검증하라",
    "rawTitle": "Evaluating LLMs Under Production Parity: A Replay Pipeline for Safe Model Swapping in Conversational Agents",
    "summary": "Hugging Face에 공개된 방법론이 새 LLM으로 갈아타기 전 실제 운영 대화를 그대로 재생(replay)해 검증한다. 8개 모델 중 3개만 통과했고, 탈락 사유는 품질이 아니라 환각이었다.",
    "bodyHtml": "<p>TechforHumans의 Anna Julia Ferreira 팀이 Hugging Face에 공개한 연구에서, LLM 공급자가 새 모델을 내놓을 때마다 겪는 곤란을 짚었습니다. 일반 벤치마크 점수가 높다고 해서 우리 서비스의 도구·말투·업무 규칙 속에서도 똑같이 동작하리란 보장은 없습니다. 그래서 이들은 승인된 실제 운영 대화를 그대로 재생하되 &lsquo;모델만&rsquo; 바꿔치우는 재생 파이프라인(replay pipeline)으로, 교체가 안전한지를 경험적으로 검증합니다.</p><blockquote>\"실제 고객을 두고 판단하는 에이전트에서 안전 기준은 가중평균에 녹여 넣는 또 하나의 항목이 아니라, 독립적인 필터여야 한다.\"<cite>TechforHumans, Hugging Face</cite></blockquote><h3>왜 중요한가</h3><p>숫자가 함정을 드러냅니다 &mdash; 평가한 8개 모델 중 승인된 것은 GPT-5.4 mini, GPT-5.4 nano, Kimi-K2.5 단 3개였습니다. 나머지 중 셋은 종합 점수가 79%를 넘고도 탈락했는데, 결정적 사유는 전반적 품질이 아니라 환각(hallucination) 발생률이 기준을 넘었다는 점이었습니다. 안전 지표를 다른 점수와 뭉뚱그려 가중평균으로 처리했다면 이 위험은 &lsquo;좋은 총점&rsquo; 뒤에 가려졌을 것입니다. 실제 고객을 다루는 에이전트에서는 이 차이가 사고와 안전을 가릅니다.</p><h3>실무 적용</h3><p>파이프라인은 세 가지 불변식을 지킵니다 &mdash; 스킬과 프롬프트는 그대로 두고, 과거의 실제 도구 응답을 재사용해 충실히 재생하며, 바꾸는 변수는 오직 모델 하나입니다. 도구 실행은 실제로 호출하지 않고 시뮬레이션해, 고객 시스템에 되돌릴 수 없는 부작용을 남기지 않습니다. 모델 교체를 앞둔 팀이라면 리더보드 점수 대신 자사의 승인된 대화 로그로 A/B 재생 테스트를 설계하고, 환각 같은 안전 지표는 총점에 섞지 말고 통과·탈락을 가르는 독립 게이트로 두는 것이 핵심입니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://huggingface.co/blog/FINAL-Bench/leadboard-drug\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: We changed one line and the benchmark score moved 0.21 AUROC</a> &mdash; 한 줄 변경으로 벤치마크 점수가 요동친 사례로, 리더보드 숫자를 곧 실력으로 믿는 위험을 드러냅니다.</li><li><a href=\"https://huggingface.co/blog/state-of-open-models-summer-2026\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: State of Open Models &mdash; Summer 2026</a> &mdash; 모델이 쏟아지는 만큼 &lsquo;안전하게 갈아타는 법&rsquo;의 표준화가 왜 필요한지 큰 그림을 짚습니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 모델 교체의 진짜 리스크가 성능 저하가 아니라 &lsquo;우리 맥락에서만 드러나는 실패&rsquo;에 있다고 봅니다. 일반 벤치마크가 아니라 자사의 실제 대화로 재생 검증하고, 안전은 평균에 섞지 않고 독립 필터로 세우는 것 &mdash; 이 규율을 갖춘 팀이 최신 모델을 두려움 없이 도입합니다.</p>",
    "source": "Hugging Face",
    "sourceUrl": "https://huggingface.co/blog/TechforHumans/evaluating-llms-under-production-parity",
    "tags": [
      "LLM Evaluation",
      "Conversational AI",
      "Model Deployment"
  ...
  {
    "id": "2026-09-01-ai-era-critique",
    "category": "design",
    "date": "2026-09-01",
    "title": "AI 시대 디자인의 핵심 역량은 비평이다",
    "rawTitle": "The Core Skill of Design in the AI Era: Critique",
    "summary": "Nielsen Norman Group이 확률적으로 움직이는 AI 시대엔 정확한 사양이 아니라 '무엇이 좋은가'를 정의하는 비평이 디자이너의 핵심 역량이 된다고 짚었다.",
    "bodyHtml": "<p>Adam Elman이 Nielsen Norman Group에서 AI 시대 디자이너가 갖춰야 할 근본 역량으로 &lsquo;비평(critique)&rsquo;을 지목했습니다. 같은 입력에 늘 같은 결과를 내던 전통 소프트웨어와 달리, AI는 확률적으로 움직여 매번 다른 출력을 내놓습니다. 그래서 디자이너의 일은 화면의 정확한 동작을 일일이 지정하는 것에서, &lsquo;좋은 결과란 무엇인가&rsquo;를 평가 기준으로 정의하는 쪽으로 옮겨갑니다.</p><blockquote>\"무엇이 &lsquo;좋은가&rsquo;, 그리고 무엇이 좋지 않은가를 정의하라.\"<cite>Adam Elman, Nielsen Norman Group</cite></blockquote><h3>왜 중요한가</h3><p>비결정성(nondeterminism)이 문제의 핵심입니다 &mdash; 출력이 매번 달라지는 시스템에서는 &lsquo;이 버튼을 누르면 이렇게 동작하라&rsquo;식의 정확한 사양이 성립하지 않습니다. 대신 디자이너는 사용자 리서치에 근거한 구체적이고 측정 가능한 기준을 세워야 합니다. 예컨대 &lsquo;답변이 장황하게 느껴지는가&rsquo;를 주관적으로 묻는 대신, 응답 유형을 먼저 분류한 뒤 그 유형별로 장황함을 평가하는 식입니다. 판단 기준이 흐릿하면 AI는 흐릿한 방향으로 표류합니다.</p><h3>실무 적용</h3><p>핵심은 &lsquo;판단 &rarr; 평가 &rarr; 반복(judge-evaluate-iterate)&rsquo;의 루프를 제품에 심는 것입니다. 명확한 판정 기준을 세우고, 모델 출력을 그 기준에 비추어 평가하고, 구현을 다듬는 과정을 돌립니다. 평가는 &lsquo;LLM을 심판(judge)으로 쓰는&rsquo; 방식으로 규모를 키울 수 있지만, 반드시 사람이 라벨링한 테스트셋으로 그 심판의 정확도를 보정해야 신뢰할 수 있습니다. 디자이너의 사용자 이해와 취향이 이 기준 안에 그대로 인코딩되는 셈입니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://www.nngroup.com/articles/eval-ai-output/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Nielsen Norman Group: One AI Output Is an Example, Not an Evaluation</a> &mdash; 출력 하나로 성능을 판단하지 말고 여러 대표 입력과 반복 실행으로 평가하라는, 비평 기준의 통계적 토대를 짚습니다.</li><li><a href=\"https://www.nngroup.com/articles/ux-context-design/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Nielsen Norman Group: UX-Context Design &mdash; Using UX Knowledge to Inform AI-Generated Design</a> &mdash; UX 지식을 AI 생성 과정 앞단에 밀어넣어 &lsquo;좋음&rsquo;의 기준을 심는 방법을 제시합니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 AI가 제작을 대신할수록 디자이너의 값이 &lsquo;만드는 손&rsquo;이 아니라 &lsquo;무엇이 좋은지 가리는 눈&rsquo;에서 나온다고 봅니다. 비평은 취향의 표현이 아니라 측정 가능한 기준을 세우는 공학입니다 &mdash; 좋음을 정의할 줄 아는 팀만이 확률적으로 흔들리는 AI를 사용자 쪽으로 붙잡아 둡니다.</p>",
    "source": "Nielsen Norman Group",
  ...
  {
    "id": "2026-09-01-agentic-rl-tito",
    "category": "ai",
    "date": "2026-09-01",
    "title": "에이전트 RL, 뱉은 토큰 그대로 학습하라",
    "rawTitle": "Agentic RL: Token-In, Token-Out Done Right",
    "summary": "Hugging Face가 도구를 쓰는 에이전트를 강화학습할 때 '디코딩한 토큰을 다시 인코딩하지 말라'는 TITO 원칙을 제시했다. 오픈 모델 19종 중 18종이 이 방식에 들어맞았다.",
    "bodyHtml": "<p>Quentin Gallou&eacute;dec와 Hugging Face 팀이 도구를 사용하는 에이전트를 강화학습(RL)으로 훈련할 때 흔히 놓치는 함정을 짚었습니다. 롤아웃 도중 대화를 다시 렌더링하면, 원래 모델이 샘플링했던 토큰과 다른 시퀀스가 만들어질 수 있습니다. 그러면 그래디언트가 정책이 실제로 생성한 적 없는 토큰을 겨냥하게 되고, 학습의 수학 자체가 깨집니다. 해법은 단순한 원칙 하나입니다 &mdash; 모델이 실제로 생성한 토큰만 학습에 쓰는 &lsquo;Token-In, Token-Out(TITO)&rsquo;.</p><blockquote>\"이미 디코딩한 토큰을 다시 인코딩하지 말라.\"<cite>Quentin Gallou&eacute;dec, Hugging Face</cite></blockquote><h3>왜 중요한가</h3><p>숫자가 실현 가능성을 보여줍니다 &mdash; 테스트한 주요 오픈 가중치 모델 계열 19종 중 18종이 TITO 구현에 필요한 &lsquo;접두어 보존(prefix-preservation)&rsquo; 성질을 만족했습니다. 즉 도구 응답을 이어 붙여도 앞선 토큰을 흐트러뜨리지 않고 시퀀스가 그대로 연장된다는 뜻입니다. 이 조건이 성립하면, 모델별로 대화 템플릿을 손으로 다시 구현하지 않고도 학습의 수학적 정합성을 지킬 수 있습니다. 재렌더링이 만들어내던 미묘한 버그가 원천에서 사라지는 셈입니다.</p><h3>실무 적용</h3><p>핵심 구현은 두 번의 템플릿 렌더 결과를 차분해 얻는 &lsquo;도구 응답 델타(tool-response delta)&rsquo;뿐입니다. 모델마다 전용 렌더러를 하드코딩할 필요 없이, 델타만 이어 붙여 정확성을 유지합니다. 에이전트형 RL 파이프라인을 직접 굴리는 팀이라면, 롤아웃과 학습 사이에서 토큰이 재인코딩되며 어긋나는지부터 점검하는 것이 안정적 수렴의 출발점입니다. 원인 모를 학습 붕괴의 상당수가 여기에서 비롯됩니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://huggingface.co/blog/LiquidAI/lfm2-5-2-6b\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: Deploy Local Agents Everywhere with LFM2.5-2.6B</a> &mdash; 온디바이스로 에이전트를 배포하려는 흐름 속에서 학습 정합성이 왜 중요한지 맥락을 더합니다.</li><li><a href=\"https://huggingface.co/blog/state-of-open-models-summer-2026\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: State of Open Models &mdash; Summer 2026</a> &mdash; 오픈 모델이 쏟아지는 만큼 &lsquo;제대로 학습시키는 법&rsquo;의 표준화가 왜 필요한지 큰 그림을 짚습니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 에이전트 시대의 실력이 화려한 데모가 아니라 &lsquo;학습이 수학적으로 맞는가&rsquo;라는 기본기에서 갈린다고 봅니다. 디코딩한 토큰을 그대로 학습에 쓰는 원칙 하나가, 모델별 땜질을 없애고 파이프라인을 단순하게 만듭니다 &mdash; 기본을 지키는 팀이 흔들리지 않는 에이전트를 훈련합니다.</p>",
    "source": "Hugging Face",
    "sourceUrl": "https://huggingface.co/blog/huggingface/tito",
    "tags": [
      "Agentic RL",
  ...
  },
  {
    ...
    "category": "design",
    "date": "2026-08-30",
    "title": "AI가 만든 뒤, UX는 청소부가 된다",
    "rawTitle": "The Custodial Era of UX: Cleaning Up After AI",
    "summary": "Nielsen Norman Group이 AI가 UX 평가보다 빠르게 만들어내면서, 디자이너가 예방보다 뒷정리에 시간을 쓰는 '관리(custodial) 시대'가 왔다고 짚었다.",
    "bodyHtml": "<p>Anna Kaley와 Raluca Budiu가 Nielsen Norman Group에서 AI 시대 UX의 새 국면을 짚었습니다. AI는 오후 한나절에 워크플로 다섯 버전을 뽑아낼 만큼 제작 속도를 끌어올렸지만, 그것이 사용자에게 정말 쓸모 있는지 판별하는 평가는 그 속도를 따라가지 못합니다. 그 결과 UX는 문제를 예방하기보다 AI가 만든 결과를 뒤늦게 청소하는 '관리(custodial) 시대'로 밀려납니다.</p><blockquote>\"빠르게 내놓는 것과 쓸모 있게 만드는 것은 다르다.\"<cite>Anna Kaley &amp; Raluca Budiu, Nielsen Norman Group</cite></blockquote><h3>왜 중요한가</h3><p>조직은 빠른 AI 제작 &rarr; 건너뛴 UX 평가 &rarr; 사용자 문제 발생 &rarr; UX의 뒷수습이라는 악순환을 반복합니다. 어떤 워크플로가 실제로 사용자의 목표·인지 부하·접근성·업무 흐름에 맞는지는 여전히 사람의 판단을 요구하는데, 이 판단을 생략한 채 '완성됐다'고 착각하면 UX 부채가 쌓입니다. 속도가 곧 품질이 아니라는 사실이 조직 차원에서 드러나는 셈입니다.</p><h3>실무 적용</h3><p>NN/g는 세 가지 대응을 제안합니다 &mdash; 무엇을 만들지에 대한 공유된 판단 기준을 세우고, 평가 속도를 제작 속도에 맞춰 끌어올리며, 조직의 디자인 지식을 AI 생성 도구 안으로 직접 밀어넣는 것입니다. 핵심은 매번 결과물을 손보는 '청소'가 아니라, 애초에 AI가 옳은 방향으로 만들도록 지식을 앞단에 심는 '예방'으로 무게를 옮기는 데 있습니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://www.nngroup.com/articles/state-of-ux-2026/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Nielsen Norman Group: State of UX 2026 &mdash; Design Deeper to Differentiate</a> &mdash; AI 시대에 UX가 차별화하려면 표면이 아니라 더 깊은 층을 설계해야 한다는 흐름을 짚습니다.</li><li><a href=\"https://www.smashingmagazine.com/2026/06/how-make-design-system-ai-ready/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Smashing Magazine: How To Make Your Design System AI-Ready</a> &mdash; 에이전트가 안전하게 작업하도록 디자인 시스템을 준비하는 방법을 제시합니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 AI가 빨라질수록 값이 '만드는 손'이 아니라 '무엇이 좋은지 가리는 눈'에서 나온다고 봅니다. 청소부로 남지 않으려면 평가를 제작만큼 빠르게 끌어올리고, 판단 기준을 앞단에 심어야 합니다 &mdash; 예방을 설계로 다루는 팀이 AI가 쏟아내는 결과 속에서 품질을 지킵니다.</p>",
    "source": "Nielsen Norman Group",
  ...
  {
    "id": "2026-08-30-granite-4-2-llms",
    "category": "ai",
    "date": "2026-08-30",
    "title": "작지만 추론하는 오픈 LLM, Granite 4.2",
    "rawTitle": "Granite 4.2 LLMs: How They're Built",
    "summary": "IBM이 3·8·30B 규모의 오픈 LLM Granite 4.2를 공개했다. 15조 토큰으로 학습해 512K 맥락과 사고/비사고 모드를 갖췄고, 30B는 SWE-Bench Verified에서 57%를 기록했다.",
    "bodyHtml": "<p>IBM이 Hugging Face에 오픈 LLM 제품군 'Granite 4.2'를 공개했습니다. 3B·8B·30B 세 가지 밀집(dense) 디코더 모델을 약 15조 토큰으로 처음부터 학습했고, 다섯 단계 사전학습으로 512K 토큰의 긴 맥락을 확보했습니다. 특징은 모든 모델이 답변 전에 '사고의 연쇄(chain of thought)'를 생성할 수 있고, 과제 난이도에 따라 사고·비사고 모드를 오간다는 점입니다.</p><blockquote>\"모든 모델은 답변에 앞서 사고의 연쇄를 생성할 수 있으며, 과제에 필요한 만큼 '사고' 또는 '비사고' 모드로 작동한다.\"<cite>Granite Team, IBM · Hugging Face</cite></blockquote><h3>왜 중요한가</h3><p>숫자가 설계 의도를 보여줍니다 &mdash; 지도 미세조정(SFT)에 약 720만 개 샘플(약 1,000억 토큰)을 쓰고, 8B·30B 모델은 실제 샌드박스 환경에서 에이전트형 강화학습을 거쳤습니다. 그 결과 30B 모델은 SWE-Bench Verified에서 57.00%를 기록했습니다. 거대 규모 경쟁이 아니라, 중형 오픈 모델이 실제 코드 수정 과제를 얼마나 풀어내는지로 승부가 옮겨가고 있음을 보여줍니다.</p><h3>실무 적용</h3><p>512K 맥락과 사고·비사고 모드를 함께 갖췄다는 것은, 긴 문서·코드베이스를 통째로 다루면서도 단순 질의에는 추론 깊이를 낮춰 비용을 아낄 수 있다는 뜻입니다. 자체 인프라에 오픈 가중치로 얹어 데이터 유출 없이 운영하려는 팀이라면, 30B로 에이전트 과제를 처리하고 3B·8B로 경량 작업을 분담하는 구성이 현실적입니다. 다만 벤치마크 수치가 자사 과제로 그대로 옮겨지는지는 직접 평가로 확인해야 합니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://huggingface.co/blog/state-of-open-models-summer-2026\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: State of Open Models &mdash; Summer 2026</a> &mdash; 오픈 모델 생태계가 소형·효율 중심으로 확장되는 큰 그림을 수치로 짚습니다.</li><li><a href=\"https://huggingface.co/blog/LiquidAI/lfm25-dspark\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: Up to 3.2x Faster Inference with LFM2.5-DSpark</a> &mdash; 같은 오픈 진영에서 추론 속도를 끌어올리는 효율 경쟁의 흐름을 보여줍니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 오픈 LLM의 가치가 '가장 큰 모델'이 아니라 '내 서버에서, 조절 가능한 비용으로, 실제 과제를 푸는가'에 있다고 봅니다. 512K 맥락과 사고 모드를 갖춘 30B급 오픈 모델은, 폐쇄형 API에 기대지 않고도 에이전트 제품을 세울 토대가 됩니다 &mdash; 효율과 통제를 함께 쥔 팀이 AI 원가를 설계합니다.</p>",
    "source": "Hugging Face",
  ...
  },
  {
    ...
    "category": "design",
    "date": "2026-08-27",
    "title": "대시보드는 데이터가 아니라 '결정'을 위한 것",
    "rawTitle": "Rethinking Data Visualisation: A UX Approach To Dashboards That Actually Drives Decisions",
    "summary": "Smashing Magazine이 대시보드가 결정을 이끌지 못하는 이유는 데이터 부족이 아니라 UX 사고의 부재라고 짚었다. 차트를 그리기 전 맥락·대상·인사이트를 정의하는 상류 작업이 성패의 8할이다.",
    "bodyHtml": "<p>Meriem Benhabiles가 Smashing Magazine에서 대시보드가 실패하는 진짜 이유를 짚었습니다. 대부분의 대시보드는 데이터가 부족해서가 아니라, 구조화된 UX 사고 없이 만들어지기 때문에 결정을 이끌지 못합니다. 데이터 시각화와 UX를 별개가 아니라 서로를 보완하는 한 쌍으로 다루면, 대시보드는 정보를 수동적으로 나열하는 화면에서 실제 판단을 돕는 도구로 바뀝니다.</p><blockquote>\"데이터는 하나의 메시지이며, 적절한 신호의 양은 전적으로 그것을 받는 사람에게 달려 있다.\"<cite>Meriem Benhabiles, Smashing Magazine</cite></blockquote><h3>왜 중요한가</h3><p>그는 대시보드 성공의 약 80%가 차트를 그리기 전의 상류 작업에서 결정된다고 말합니다 &mdash; 어떤 운영상의 질문에 답하는지(맥락), 누가 어떻게 사고하며 쓰는지(대상), 이 화면을 보고 어떤 결정이나 변화가 일어나야 하는지(인사이트)를 먼저 못 박는 일입니다. 이 세 질문을 건너뛰면 아무리 정확한 데이터도 그저 '보기 좋은 표'로 남습니다.</p><h3>실무 적용</h3><p>핵심 처방은 '무조건 단순하게'가 아니라 '적정 복잡도'입니다 &mdash; 데이터 밀도는 보는 사람의 데이터 리터러시와 책임 범위에 맞춰야 하고, 맥락을 덜어내다 보면 판단에 꼭 필요한 층까지 사라집니다. 정확함만으로는 행동이 나오지 않으므로, 필요한 변수를 정확히 수면 위로 끌어올리도록 대시보드를 '설계'해야 합니다. 정보(information)와 인사이트(insight)는 다릅니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://www.nngroup.com/articles/dashboards-preattentive/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Nielsen Norman Group: Dashboards &mdash; Making Charts and Graphs Easier to Understand</a> &mdash; 한눈에 읽히도록 전주의적(preattentive) 시각 처리에 맞춘 차트 설계를 짚습니다.</li><li><a href=\"https://www.nngroup.com/articles/complex-application-design/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Nielsen Norman Group: 8 Design Guidelines for Complex Applications</a> &mdash; 중요한 정보를 시각적으로 부각하고 정보 위계를 다루는 원칙을 제시합니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 좋은 대시보드의 승부가 '무엇을 보여주느냐'가 아니라 '어떤 결정을 돕느냐'에서 갈린다고 봅니다. 차트를 예쁘게 다듬기 전에 맥락·대상·인사이트를 먼저 정의하는 팀만이, 화면을 보고 행동으로 이어지는 도구를 만듭니다 &mdash; 데이터 시각화는 미학이 아니라 의사결정 설계의 문제입니다.</p>",
    "source": "Smashing Magazine",
    "sourceUrl": "https://www.smashingmagazine.com/2026/08/rethinking-data-visualisation-ux-approach-dashboards/",
    "tags": [
      "Data Visualization",
      "Dashboards",
      "UX Strategy"
  ...
  {
    "id": "2026-08-27-icml-open-reproductions",
    "category": "ai",
    "date": "2026-08-27",
    "title": "AI 에이전트가 논문 2,226편을 재현하다",
    "rawTitle": "What We Learned by Reproducing 2,200 papers from ICML",
    "summary": "Hugging Face의 ICML 2026 오픈 재현 챌린지에서 커뮤니티가 AI 코딩 에이전트로 채택 논문의 34%를 재현했다. 절반은 주장이 검증됐지만 23%는 반증·논쟁에 부딪혔다.",
    "bodyHtml": "<p>Hugging Face가 ICML 2026 채택 논문을 AI 코딩 에이전트로 재현하는 커뮤니티 챌린지의 결과를 공개했습니다. 7월 15일부터 8월 2일까지 19일간 1,221명이 참여해 6,816개의 재현 로그북을 남겼고, 채택 논문의 약 34%에 해당하는 2,226편을 다뤘습니다 &mdash; 이 과정에서 논문이 내세운 개별 주장 35,908개가 독립적으로 평가됐습니다.</p><blockquote>\"내 신뢰도 점수가 낮은 이유는 모든 증명을 꼼꼼히 확인하지 못했기 때문이다.\"<cite>ICML 2026 스포트라이트 논문 리뷰어, Hugging Face</cite></blockquote><h3>왜 중요한가</h3><p>숫자가 학계의 현실을 드러냅니다 &mdash; 검토된 논문의 51%는 적어도 하나의 주장이 독립적으로 검증됐지만, 23%는 적어도 하나의 주장이 반증되거나 논쟁에 부딪혔습니다. 오류가 있는 수학적 증명, 이론과 구현의 불일치, 패딩으로 부풀린 평가 지표 같은 사례가 대표적입니다. 동료 심사를 통과한 논문이라도 핵심 주장이 재현 앞에서 흔들릴 수 있다는 뜻입니다.</p><h3>실무 적용</h3><p>이 실험은 AI 에이전트의 강점과 한계를 동시에 보여줍니다 &mdash; 에이전트는 실험을 실행하는 데는 탁월하지만, 규모에 따라 달라지는 동작을 다루거나 방향을 스스로 잡는 데는 여전히 사람의 감독이 필요합니다. 연구·제품 팀이라면 '재현 가능성 검증'을 사람만의 일이 아니라 에이전트로 대량화할 수 있는 파이프라인으로 보고, 대신 무엇을 검증할지와 결과 판정은 사람이 쥐는 하이브리드가 현실적입니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://huggingface.co/blog/asr-benchmark-optimization\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: Measuring benchmark optimization in speech recognition</a> &mdash; 벤치마크 점수를 좇다 실제 성능을 왜곡하는 '벤치맥싱' 문제를 짚습니다.</li><li><a href=\"https://huggingface.co/blog/state-of-open-models-summer-2026\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: State of Open Models &mdash; Summer 2026</a> &mdash; 오픈 모델·데이터가 폭증하는 생태계의 큰 그림을 수치로 보여줍니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 AI가 연구를 '더 빨리 쓰는' 단계를 넘어 '더 빨리 검증하는' 단계로 넘어가고 있다고 봅니다. 논문의 4분의 1이 재현 앞에서 흔들린다면, 앞으로의 신뢰는 '누가 발표했는가'가 아니라 '기계가 재현했는가'에서 나옵니다 &mdash; 검증을 자동화하되 판단은 사람이 쥐는 팀이 진짜 지식을 가려냅니다.</p>",
    "source": "Hugging Face",
  ...
  {
    "id": "2026-08-26-website-never-stop-changing",
    "category": "design",
    "date": "2026-08-26",
    "title": "웹사이트는 멈추는 순간 낡는다",
    "rawTitle": "Why Your Website Should Never Stop Changing",
    "summary": "Smashing Magazine이 웹사이트는 기술 고장이 아니라 방치로 낡는다고 짚었다. AI 에이전트로 상시 유지하되, 자율성은 전면 위임이 아니라 사람·업무별로 잘게 조절하라는 제안이다.",
    "bodyHtml": "<p>Strapi 공동창업자 Pierre Burgy가 Smashing Magazine에서 웹사이트가 낡는 진짜 이유를 짚었습니다. 대부분의 사이트는 출시 첫날이 가장 좋고, 이후 시장이 바뀌고 업데이트가 밀리면서 서서히 뒤처집니다 &mdash; 서버가 죽어서가 아니라 아무도 손대지 않아서입니다. 그는 자율적 AI 에이전트로 사이트를 상시 유지하되, '완전 자율'을 목표로 삼는 것은 오히려 잘못이라고 말합니다.</p><blockquote>\"자율성의 목적은 당신을 웹사이트에서 밀어내는 것이 아니라, 방치로 인한 쇠퇴를 걷어내는 것이었다.\"<cite>Pierre Burgy, Smashing Magazine</cite></blockquote><h3>왜 중요한가</h3><p>그는 사이트 유지 업무를 세 층으로 나눕니다 &mdash; 접근성 점검·디자인 시스템 전파·깨진 링크 정리처럼 규칙으로 처리되는 반복 작업(약 80%), 브랜드와 창의적 판단이 필요한 소수의 '취향' 영역, 그리고 역할에 따라 자율 수준이 달라지는 중간 지대입니다. 핵심은 하나의 만능 '자율 스위치'가 아니라, 업무마다 사람마다 신뢰 수준에 맞춰 에이전트를 조립하는 개인화된 통제라는 점입니다.</p><h3>실무 적용</h3><p>실전 처방은 단순합니다 &mdash; 좁은 작업부터 에이전트에 위임하고, 결과를 눈으로 확인하며, 신뢰가 쌓이는 만큼 권한을 넓혀갑니다. 이때 신뢰는 투명성에서 나옵니다. 에이전트가 무엇을 어떻게 바꿨는지 로그와 전후 기록을 남겨 역량을 증명해야 권한이 확장됩니다. 자율의 경계선은 '기술이 허용하는 지점'이 아니라 '우리 팀의 진짜 가치가 있는 지점'에 그어야 합니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://www.smashingmagazine.com/2026/06/how-make-design-system-ai-ready/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Smashing Magazine: How To Make Your Design System AI-Ready</a> &mdash; 에이전트가 안전하게 작업하려면 디자인 시스템을 어떻게 준비해야 하는지 짚습니다.</li><li><a href=\"https://www.nngroup.com/articles/prove-framework/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Nielsen Norman Group: How to Decide When an AI Tool Is Worth Keeping (PROVE)</a> &mdash; AI 도구를 언제 신뢰하고 유지할지 판단하는 프레임을 제시합니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 웹사이트를 '완성하는 프로젝트'가 아니라 '운영하는 유기체'로 봅니다. AI 에이전트는 사람을 대체하는 도구가 아니라 쇠퇴를 막는 유지 장치이며, 신뢰는 한꺼번에 넘기는 것이 아니라 로그와 결과로 조금씩 벌어가는 것입니다 &mdash; 자율성을 잘게 나눠 통제할 줄 아는 팀이 사이트를 오래 살립니다.</p>",
    "source": "Smashing Magazine",
    "sourceUrl": "https://www.smashingmagazine.com/2026/08/why-website-should-never-stop-changing/",
    "tags": [
      "Web Maintenance",
      "AI Agents",
      "Design Ops"
  ...
  {
    "id": "2026-08-26-granite-speech-turbo-ctc",
    "category": "ai",
    "date": "2026-08-26",
    "title": "3.5시간 음성을 1초에 받아쓰다",
    "rawTitle": "Extremely Fast and Accurate Transcription with Granite Speech 5.0 Turbo CTC",
    "summary": "IBM이 4.7억 파라미터의 초경량 음성 인식 모델 Granite Speech 5.0 Turbo CTC를 공개했다. 정확도를 지키면서 배치 추론으로 3.5시간 넘는 음성을 1초에 받아쓴다.",
    "bodyHtml": "<p>IBM이 Hugging Face에 초경량 음성 인식 모델 'Granite Speech 5.0 Turbo CTC' 두 종(Apache 2.0 라이선스판과 비상업 라이선스판)을 공개했습니다. 파라미터는 4.7억 개에 불과하지만, 속도와 정확도를 동시에 노린 인코더 전용 설계가 특징입니다 &mdash; 16개의 Conformer 블록에 청크 단위 어텐션과 CTC 손실을 결합하고, 2배 서브샘플링을 세 단계로 쌓아 초당 12.5토큰으로 처리합니다.</p><blockquote>\"전례 없는 속도와 강력한 정확도를 최소한의 크기로 결합해, 엣지 기기의 음성-텍스트 작업에 이상적이다.\"<cite>IBM Granite, Hugging Face</cite></blockquote><h3>왜 중요한가</h3><p>숫자가 설계의 성패를 말해줍니다 &mdash; NVIDIA H200 GPU에서 12,600 RTFx를 넘겨, 배치 추론이면 3.5시간이 넘는 음성을 단 1초에 받아씁니다. 그러면서 평균 단어 오류율(WER)은 4.85%(비상업판)·5.00%(Apache판)로, FFASR 리더보드에서 가장 빠른 모델이면서도 정확도 5위·9위에 올랐습니다. 속도를 위해 정확도를 크게 희생하는 통념을 4.7억 파라미터 규모에서 뒤집은 셈입니다.</p><h3>실무 적용</h3><p>이 조합은 클라우드에 음성을 보내지 않고 기기 안에서 처리해야 하는 상황 &mdash; 회의록·자막·음성 명령·콜센터 로그 &mdash; 에 특히 유효합니다. Apache 2.0 판은 상업적 활용이 자유로워 제품에 바로 얹을 수 있고, 인코더 전용·소형이라 GPU 비용과 지연을 함께 줄입니다. 대규모 아카이브를 일괄 전사하거나 실시간 자막을 붙이는 팀이라면, 거대 모델 대신 이런 경량 모델로 처리량을 확보하는 편이 현실적입니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://huggingface.co/blog/LiquidAI/lfm2-5-vl-3b\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: LFM2.5-VL-3B for Better and Faster Vision Capabilities for the Edge</a> &mdash; 엣지에서 빠르게 도는 소형 멀티모달 모델의 흐름을 보여줍니다.</li><li><a href=\"https://huggingface.co/blog/state-of-open-models-summer-2026\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: State of Open Models &mdash; Summer 2026</a> &mdash; 오픈 모델 생태계가 소형·효율 중심으로 확장되는 큰 그림을 짚습니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 음성 AI의 승부가 '가장 정확한 거대 모델'이 아니라 '어디서, 얼마나 싸게 돌아가느냐'로 넘어가고 있다고 봅니다. 4.7억 파라미터가 3.5시간을 1초에 받아쓴다는 것은, 실시간·온디바이스 전사가 사치가 아니라 기본값이 된다는 신호입니다 &mdash; 작고 빠른 모델이 제품의 체감 속도를 바꿉니다.</p>",
    "source": "Hugging Face",
    "sourceUrl": "https://huggingface.co/blog/ibm-granite/granite-speech-5-0-470m-turboctc",
    "tags": [
      "Speech Recognition",
      "Edge AI",
      "Open Models"
  ...
  },
  {
    ...
    "category": "design",
    "date": "2026-08-24",
    "title": "접근성은 기능이 아니라 '운영 역량'이다",
    "rawTitle": "Why Accessibility Is An Operational Capability, Not A Feature",
    "summary": "Smashing Magazine이 접근성을 일회성 감사가 아닌, 보안·안정성처럼 상시 굴려야 하는 운영 역량으로 다루라고 짚었다. AI가 만든 코드는 기본값이 비접근성이라 더 그렇다.",
    "bodyHtml": "<p>Smashing Magazine의 Mikhail Prosmitskiy가 접근성(accessibility)을 바라보는 관점 자체를 바꾸라고 제안했습니다. 출시 직전 한 번 점검하는 '기능'이나 컴플라이언스 체크박스가 아니라, 보안·안정성처럼 개발 과정 내내 상시 유지해야 하는 '운영 역량'으로 다루라는 것입니다. 특히 AI가 생성한 UI 코드는 기본값이 비접근성이라, 이를 방치하면 접근성 부채가 눈덩이처럼 불어난다고 경고합니다.</p><blockquote>\"디자인 리뷰에서 잡힌 접근성 문제는 코멘트 한 줄이지만, 프로덕션에서 발견되면 별도의 개선 프로젝트가 된다.\"<cite>Mikhail Prosmitskiy, Smashing Magazine</cite></blockquote><h3>왜 중요한가</h3><p>수치가 문제의 규모를 말해줍니다 &mdash; 2026년 기준 상위 100만 홈페이지의 95.9%에서 WCAG 위반이 감지됐고, 페이지당 평균 오류는 56.1개였습니다. 여기에 AI 생성 UI는 20~30줄짜리 컴포넌트 하나에도 접근성 실패가 10개 넘게 섞여 들어오기 일쑤입니다. 문제를 늦게 잡을수록 비용은 기하급수적으로 커지므로, '나중에 감사로 몰아서'는 이제 통하지 않는 전략입니다.</p><h3>실무 적용</h3><p>접근성을 디자인 시스템·엔지니어링 워크플로·CI/CD 파이프라인에 아예 심어, 코드가 병합되기 전에 자동으로 걸러지게 만드는 것이 핵심입니다. 이는 선의의 문제가 아니라 사업의 문제이기도 합니다 &mdash; 조직의 75%가 디지털 제품을 구매할 때 접근성 증빙을 요구하고, 장애인 시장은 전 세계에서 약 8조 달러의 가처분소득을 움직이며, 영국에서만 접근 불가로 이탈하는 '클릭어웨이 파운드'가 연 171억 파운드에 달합니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://master.dev/blog/ai-generated-ui-is-inaccessible-by-default/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Master.dev: AI-Generated UI Is Inaccessible by Default</a> &mdash; 왜 AI 코드가 접근성을 놓치는지 구체적 사례로 보여줍니다.</li><li><a href=\"https://www.smashingmagazine.com/2026/08/eu-guidelines-ai-labelling/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Smashing Magazine: New EU Guidelines For AI Labelling</a> &mdash; 접근성과 함께 규제 리스크로 다가오는 AI 표기 의무를 정리합니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 접근성을 '착한 기능'이 아니라 '품질 지표'로 봅니다. 보안 취약점을 배포 후에 패치하지 않듯, 접근성도 파이프라인에서 자동으로 막아야 부채가 쌓이지 않습니다 &mdash; AI가 코드를 더 빨리 찍어낼수록, 접근성을 상시 검증하는 팀만이 그 속도를 안전하게 감당할 수 있습니다.</p>",
    "source": "Smashing Magazine",
    "sourceUrl": "https://www.smashingmagazine.com/2026/06/why-accessibility-operational-capability-not-feature/",
    "tags": [
      "Accessibility",
      "Design Ops",
      "AI Code"
  ...
  {
    "id": "2026-08-24-gemma-one-billion-downloads",
    "category": "ai",
    "date": "2026-08-24",
    "title": "젬마, 다운로드 10억 돌파",
    "rawTitle": "Inside the Gemmaverse: Celebrating one billion Gemma downloads",
    "summary": "Google의 오픈 모델 Gemma가 누적 다운로드 10억을 넘겼다. 10만 개 넘는 파생 모델이 우주·의료·헬스케어까지 파고들며 '젬마버스' 생태계를 이뤘다.",
    "bodyHtml": "<p>Google이 자사 오픈 웨이트 모델 계열 Gemma의 누적 다운로드가 10억 건을 넘어섰다고 발표했습니다. 단순한 숫자 자랑이 아니라, 개발자들이 만든 파생 모델이 10만 개를 돌파하며 하나의 생태계 &mdash; 이른바 '젬마버스(Gemmaverse)' &mdash; 를 형성했다는 점이 핵심입니다. 여기에 맞춰 Google은 커뮤니티의 우수 프로젝트·파인튜닝·튜토리얼을 모은 공식 디렉터리 'Awesome Gemma' 저장소도 GitHub에 열었습니다.</p><blockquote>\"지난 2년간 개발자들이 10만 개가 넘는 Gemma 모델 변형을 공개하며 번성하는 혁신 생태계를 만들었다.\"<cite>Google</cite></blockquote><h3>왜 중요한가</h3><p>Gemma의 강점은 규모가 아니라 '어디까지 스며들었는가'에 있습니다. NASA·Satlyt·Starcloud는 궤도 위 위성에서 온보드 이미지 분석과 통신에 Gemma를 돌리고 있고, 의료에서는 C2S-Scale이 살아있는 세포에서 검증된 새로운 암 치료 경로를 찾아냈으며 MedGemma는 인도 AIIMS의 환자 분류와 우간다 지역 보건 인력을 돕습니다. 인도의 국민 건강 앱 Aarogya Setu 2.0(다운로드 1억+)은 의료 데이터 처리에 Gemma 4를 통합했습니다 &mdash; 오픈 모델이 실험실을 넘어 실제 인프라가 되었다는 증거입니다.</p><h3>실무 적용</h3><p>오픈 웨이트 모델을 검토하는 팀에게 시사점은 분명합니다 &mdash; 승부는 최고 성능 단일 모델이 아니라, 내 도메인에 맞게 얼마나 쉽게 파인튜닝·배포할 수 있는가로 넘어갑니다. 10만 개의 파생 모델이 존재한다는 것은, 처음부터 학습하지 않아도 이미 누군가 내 문제와 비슷한 버전을 만들어 뒀을 가능성이 크다는 뜻입니다. 모델 선택 시 라이선스와 커뮤니티 생태계의 두께를 성능 지표만큼 비중 있게 봐야 하는 이유입니다.</p><h3>교차 참고</h3><ul><li><a href=\"https://huggingface.co/blog/state-of-open-models-summer-2026\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Hugging Face: State of Open Models &mdash; Summer 2026</a> &mdash; 오픈 모델 저장소가 수백만 단위로 불어나는 흐름을 수치로 보여줍니다.</li><li><a href=\"https://blog.google/innovation-and-ai/technology/ai/google-io-2026-all-our-announcements/\" target=\"_blank\" rel=\"noopener noreferrer nofollow\">Google: 100 things we announced at Google I/O 2026</a> &mdash; Gemma를 둘러싼 Google AI 스택의 큰 그림을 짚어줍니다.</li></ul><h3>Wemeet의 관점</h3><p>Wemeet은 이 10억이라는 숫자보다 '10만 개의 파생 모델'이 더 중요한 신호라고 봅니다. 오픈 모델의 진짜 해자는 성능이 아니라 생태계이며, 사람들이 그 위에 무언가를 계속 쌓을 때 모델은 비로소 플랫폼이 됩니다 &mdash; 개방성이 곧 채택률로, 채택률이 곧 지속 가능성으로 이어집니다.</p>",
    "source": "Google",
    "sourceUrl": "https://blog.google/innovation-and-ai/technology/developers-tools/gemma-one-billion-downloads/",
    "tags": [
      "Open Models",
      "Gemma",
      "AI Ecosystem"
